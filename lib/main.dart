...
Future<List> getCurrencies() async {
  ...
  //sending HTTP GET request to CoinMarketCap API
...
class CryptoListWidget extends StatelessWidget {
  ...
  Widget _getAppTitleWidget() {
    return new Text(
      'Crytpocurrencies', style: new TextStyle(color: Colors.white,
  ...
  Widget _getListViewWidget() {
    return new Flexible(
        child: new ListView.builder(itemCount: _currencies.length,
  ...
  Widget _buildBody() {
    return new Container(margin: const EdgeInsets.fromLTRB(8.0, 56.0, 8.0, 0.0),
      child: new Column(
  ...
  Widget build(BuildContext context) {
    return new Scaffold(body: _buildBody(), backgroundColor: Colors.blue,);
  }
}